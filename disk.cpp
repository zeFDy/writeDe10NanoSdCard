...
HANDLE getHandleOnFile(LPCWSTR filelocation, DWORD access)
{
    ...
    if (hFile == INVALID_HANDLE_VALUE)
    {
        ...
        printf("An error occurred when attempting to get a handle on the file.\n");
...
DWORD getDeviceID(HANDLE hVolume)
{
    ...
    if (!DeviceIoControl(hVolume, IOCTL_VOLUME_GET_VOLUME_DISK_EXTENTS, NULL, 0, &sd, sizeof(sd), &bytesreturned, NULL))
    {
        ...
        //                                  "Error %1: %2").arg(GetLastError()).arg(errText));
        printf("An error occurred when attempting to get information on volume.\n");
        //LocalFree(errormessage);
    }
...
HANDLE getHandleOnDevice(int device, DWORD access)
{
    ...
    memset(caDeviceName, 0, 256);
    sprintf(caDeviceName, "\\\\.\\PhysicalDrive%d", device);
    //QString devicename = QString("\\\\.\\PhysicalDrive%1").arg(device);
    
    CString aCString = CString(caDeviceName);
    hDevice = CreateFile((LPCWSTR)aCString/*caDeviceName*//*devicename.toLatin1().data()*/, access, FILE_SHARE_READ | FILE_SHARE_WRITE, NULL, OPEN_EXISTING, 0, NULL);
    if (hDevice == INVALID_HANDLE_VALUE)
    {
        ...
        //                                  "Error %1: %2").arg(GetLastError()).arg(errText));
        printf("An error occurred when attempting to get a handle on the device.\n");
        //LocalFree(errormessage);
    }
    return hDevice;
}
...
HANDLE getHandleOnVolume(int volume, DWORD access)
{
    ...
    char volumename[] = "\\\\.\\A:";
    volumename[4] += volume;
    //const char volumename[] = "\\\\.\\H:";
    
    /*
    //const char * cs = <your input char*>
    size_t wn = mbsrtowcs(NULL, (const char**)&volumename, 0, NULL);

    // error if wn == size_t(-1)
    wchar_t * buf = new wchar_t[wn + 1]();  // value-initialize to 0 (see below)
    wn = mbsrtowcs(buf, (const char**)&volumename, wn + 1, NULL);
  ...
  */
    CString aCString = CString(volumename);

    hVolume = CreateFile((LPCTSTR) aCString/*(LPCWSTR)volumename*//*buf*/, access, FILE_SHARE_READ | FILE_SHARE_WRITE, NULL, OPEN_EXISTING, 0, NULL);
    if (hVolume == INVALID_HANDLE_VALUE)
    {
        ...
        //                                  "Error %1: %2").arg(GetLastError()).arg(errText));
        printf("An error occurred when attempting to get a handle on the volume.\n");
        //LocalFree(errormessage);
    }
    //delete[] buf;
    return hVolume;
...
bool getLockOnVolume(HANDLE handle)
{
    ...
    if (!bResult)
    {
        ...
        //                                  "Error %1: %2").arg(GetLastError()).arg(errText));
        printf("An error occurred when attempting to lock the volume.\n");
        //LocalFree(errormessage);
    }
    return (bResult);
}
...
bool removeLockOnVolume(HANDLE handle)
{
    ...
    if (!bResult)
    {
        ...
        //                                  "Error %1: %2").arg(GetLastError()).arg(errText));
        printf("An error occurred when attempting to unlock the volume.\n");
        //LocalFree(errormessage);
    }
    return (bResult);
}
...
bool unmountVolume(HANDLE handle)
{
    ...
    if (!bResult)
    {
        ...
        //                                  "Error %1: %2").arg(GetLastError()).arg(errText));
        printf("An error occurred when attempting to dismount the volume.\n");
        //LocalFree(errormessage);
    }
    return (bResult);
}
...
char *readSectorDataFromHandle(HANDLE handle, unsigned long long startsector, unsigned long long numsectors, unsigned long long sectorsize)
{
    ...
    if (!ReadFile(handle, data, sectorsize * numsectors, &bytesread, NULL))
    {
        ...
        //                                  "Error %1: %2").arg(GetLastError()).arg(errText));
        printf("An error occurred when attempting to read data from handle.\n");
        //LocalFree(errormessage);
        delete[] data;
        data = NULL;
    }
...
bool writeSectorDataToHandle(HANDLE handle, char *data, unsigned long long startsector, unsigned long long numsectors, unsigned long long sectorsize)
{
    ...
    if (!bResult)
    {
        ...
        //                                  "Error %1: %2").arg(GetLastError()).arg(errText));
        printf("An error occurred when attempting to write data to handle.\n");
        //LocalFree(errormessage);
    }
    return (bResult);
}
...
unsigned long long getNumberOfSectors(HANDLE handle, unsigned long long *sectorsize)
{
    ...
    if (!bResult)
    {
        ...
        //                                  "Error %1: %2").arg(GetLastError()).arg(errText));
        printf("An error occurred when attempting to get the device's geometry.\n");
        //LocalFree(errormessage);
        return 0;
    }
    if (sectorsize != NULL)
...
unsigned long long getFileSizeInSectors(HANDLE handle, unsigned long long sectorsize)
{
    ...
    if (sectorsize) // avoid divide by 0
    {
        ...
        if(GetFileSizeEx(handle, &filesize) == 0)
        {
            ...
            //                                  "Error %1: %2").arg(GetLastError()).arg(errText));
            printf("An error occurred while getting the file size.\n");
            //LocalFree(errormessage);
            retVal = 0;
        }
        else
...
bool spaceAvailable(char *location, unsigned long long spaceneeded)
{
    ...
    CString aCString = CString(location);
    
    bResult = GetDiskFreeSpaceEx((LPCWSTR)aCString/*location*/, NULL, NULL, &freespace);
    if (!bResult)
    {
        //wchar_t *errormessage=NULL;
        //FormatMessageW(FORMAT_MESSAGE_FROM_SYSTEM | FORMAT_MESSAGE_ALLOCATE_BUFFER, NULL, GetLastError(), 0, (LPWSTR)&errormessage, 0, NULL);
        ...
        printf("Failed to get the free space on drive.\n");
        return true;
    }
                      ...
                      DEVICE_NUMBER *devInfo)
{
    ...
    if (bResult)
    {
        ...
        if (!bResult)
        {
            ...
            printf("An error occurred while getting the device number.\n");
            //LocalFree(errormessage);
        }
    }
    else
    {
                            ...
                            (LPOVERLAPPED) NULL))
        {
            ...
            //                                  "This usually means something is currently accessing the device;"
            //                                  " please close all applications and try again.\n\nError %1: %2").arg(GetLastError()).arg(errText));
            printf("An error occurred while querying the properties.\n");
            //LocalFree(errormessage);
        }
            retVal = false;
    }
...
bool checkDriveType(char *name, ULONG *pid)
{
    ...
    CString aCSnameWithSlash = CString(nameWithSlash);
    driveType = GetDriveType((LPCWSTR)aCSnameWithSlash/*nameWithSlash*/);
    CString aCSnameNoSlash = CString(nameNoSlash);
    ...
    case DRIVE_FIXED:     // The media cannot be removed from the drive. Some USB drives report as this.
        hDevice = CreateFile((LPCWSTR)aCSnameNoSlash/*nameNoSlash*/, FILE_READ_ATTRIBUTES, FILE_SHARE_READ | FILE_SHARE_WRITE, NULL, OPEN_EXISTING, 0, NULL);
        if (hDevice == INVALID_HANDLE_VALUE)
        {
            //wchar_t *errormessage=NULL;
            //FormatMessageW(FORMAT_MESSAGE_FROM_SYSTEM | FORMAT_MESSAGE_ALLOCATE_BUFFER, NULL, GetLastError(), 0, (LPWSTR)&errormessage, 0, NULL);
            ...
            printf("An error occurred when attempting to get a handle\n");
            //LocalFree(errormessage);
        }
        else
        {
                      ...
                      || (pDevDesc->BusType == BusTypeSd ) || (pDevDesc->BusType == BusTypeMmc )) ) ) )
            {
                ...
                {
                    CloseHandle(hDevice);
                    hDevice = CreateFile((LPCWSTR)aCSnameNoSlash, FILE_READ_DATA, FILE_SHARE_READ | FILE_SHARE_WRITE, NULL, OPEN_EXISTING, 0, NULL);
